...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update contact
    $stmt = $pdo->prepare("UPDATE PERSON SET FirstName = ?, LastName = ?, Email = ?, DOB = ?, image_path = ?, fav = ? WHERE id = ?");
    $stmt->execute([$firstName, $lastName, $email, $dob, $imagePath, $fav, $contactId]);
    ...
    if (isset($_POST['phone_numbers'])) {
        foreach ($_POST['phone_numbers'] as $phone) {
            if (!empty($phone['number'])) {
                ...
                $stmt->execute([$contactId, $phone['number'], $phone['label']]);
            }
        }
    }
    
    header("Location: view.php?id=" . $contactId);
    exit;
}
?>
...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="edit.php?id=<?= $contact['id'] ?>" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex items-center space-x-4">
                    <div id="avatar-preview" class="w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                        <?php if($contact['image_path']): ?>
                            <img src="<?= htmlspecialchars($contact['image_path']) ?>" class="w-full h-full object-cover">
                        <?php else: ?>
                            <?= generateAvatar($contact['FirstName'], 64) ?>
                        <?php endif; ?>
                    </div>
                    <div>
            ...
            <div>
                ...
                <input type="text" id="first_name" name="first_name" required class="w-full border rounded p-2" value="<?= htmlspecialchars($contact['FirstName']) ?>">
            </div>
            
            <div>
                <label for="last_name" class="block text-gray-700 mb-2">Last Name</label>
                <input type="text" id="last_name" name="last_name" class="w-full border rounded p-2" value="<?= htmlspecialchars($contact['LastName']) ?>">
            </div>
            
            <!-- Email and DOB -->
            <div>
                <label for="email" class="block text-gray-700 mb-2">Email</label>
                <input type="email" id="email" name="email" class="w-full border rounded p-2" value="<?= htmlspecialchars($contact['Email']) ?>">
            </div>
            
            <div>
                <label for="dob" class="block text-gray-700 mb-2">Date of Birth</label>
                <input type="date" id="dob" name="dob" class="w-full border rounded p-2" value="<?= htmlspecialchars($contact['DOB']) ?>">
            </div>
                ...
                <div id="phone-numbers-container">
                    <?php foreach($phoneNumbers as $index => $phone): ?>
                        <div class="phone-number-group flex space-x-2 mb-2">
                            <select name="phone_numbers[<?= $index ?>][label]" class="border rounded p-2">
                                <?php $labels = ['Primary', 'Secondary', 'Work', 'Home', 'Other']; ?>
                                <?php foreach($labels as $label): ?>
                                    <option value="<?= $label ?>" <?= $phone['label'] == $label ? 'selected' : '' ?>><?= $label ?></option>
                                <?php endforeach; ?>
                            </select>
                            <input type="tel" name="phone_numbers[<?= $index ?>][number]" class="border rounded p-2 flex-grow" value="<?= htmlspecialchars($phone['PhoneN']) ?>" placeholder="Phone number">
            ...
            <div class="md:col-span-2 flex items-center">
                <input type="checkbox" id="fav" name="fav" class="mr-2" <?= $contact['fav'] == 'Y' ? 'checked' : '' ?>>
                <label for="fav" class="text-gray-700">Mark as favorite</label>
            </div>
        </div>
        
        <div class="mt-6 flex justify-end space-x-3">
            <a href="view.php?id=<?= $contact['id'] ?>" class="bg-gray-300 hover:bg-gray-400 px-4 py-2 rounded">Cancel</a>
            <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">Update Contact</button>
        </div>
    </form>
</div>

<script>
let phoneCount = <?= count($phoneNumbers) > 0 ? count($phoneNumbers) : 1 ?>;
